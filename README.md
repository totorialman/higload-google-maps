...
Используется **Geo-Based DNS**:  
- Пользователь при резолвинге домена направляется в ближайший ДЦ.  
- DNS-записи обновляются через **Anycast CDN**, чтобы учесть динамику нагрузки.  
- Для отказоустойчивости предусмотрен fallback на соседние ДЦ в пределах региона.  

---

## Схема Anycast-балансировки

**Anycast IP** назначается для всех дата-центров. Особенности:  
- Один IP доступен из разных точек мира, маршрутизация BGP ведёт запрос к ближайшему узлу.  
- Снижается задержка за счёт выбора самого короткого сетевого пути.  
- Поддержка защиты от DDoS (распределение трафика между множеством точек при атаке).  
- В США, где сосредоточены сразу три ДЦ, Anycast используется для внутреннего балансинга между узлами.  
