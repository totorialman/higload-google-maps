...
| **place**    | **R:** **Информация о местах** + **Поиск мест** = `67.8K`<br/>**W:** Пользовательские правки карт — **редкая операция**. По нашим оценкам: ~2 RPS глобально (см. ниже).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 67.8K    | 2         |
...
> **Важно**: Все операции записи (`review`, `place`, `map_version`) направляются **только в мастер-БД в Нью-Йорке**. RO-регионы получают данные через **асинхронную репликацию** с задержкой < 1 сек. Поэтому Write QPS в таблицах выше относится **исключительно к мастер-ноде**.
