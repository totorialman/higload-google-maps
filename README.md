...
### MetaTile: оптимизация рендеринга тайлов

**MetaTile** — это архитектурная техника, при которой вместо рендеринга отдельных тайлов размером 256×256 пикселей сервер генерирует единый крупный тайл (например, 4×4 = 1024×1024 пикселей), а затем разрезает его на стандартные тайлы. Эта стратегия применяется только на этапе генерации, не нарушая стандарт XYZ, по которому клиенты (браузеры, мобильные приложения) ожидают получать тайлы именно 256×256 px.

### Проблема: почему нельзя рендерить тайлы по одному?

При рендеринге каждого тайла отдельно возникают проблемы:

1. **Высокая нагрузка на рендерер и БД**  
   Каждый тайл требует:
   - Отдельного запроса к PostgreSQL.
   - Отдельного вызова Mapnik для рендеринга.
   - Отдельной записи в кэш.  

2. **Визуальные артефакты на границах**  
   Mapnik не знает о соседних тайлах, поэтому:
   - Подписи могут дублироваться или обрезаться.
   - Линии дорог и рек рвутся на стыках.
   - POI-объекты могут отображаться дважды или пропадать.


### Решение: как работает MetaTile

1. **Определение ключа MetaTile**  
   При запросе тайла `/z/x/y.png` сервер вычисляет координаты MetaTile размером 4×4.  

2. **Проверка кэша**  
   Выполняется запрос к таблице `tile_cache` в PostgreSQL.

3. **Ленивый рендеринг (lazy rendering)**  
   Если запись отсутствует или устарела:
   - Выполняется **один запрос к PostgreSQL** за всеми геообъектами в области 4×4 тайлов.
   - **Mapnik** генерирует единое изображение 1024×1024 px.
   - Результат **разрезается на 16 тайлов** 256×256 px.

4. **Сохранение**  
   - 16 тайлов сохраняются во внешнее хранилище GCS в формате `.webp`.
   - В PostgreSQL (`tile_cache`) создаются 16 записей с метаданными: `file_path`, `rendered_at`, `version_hash`, `expires_at`.

5. **Отдача клиенту**  
   Клиент получает **только запрошенный тайл** через CDN, как обычно.
...
| TypeScript       | Frontend                        | Скриптовый язык со статической типизацией                                                                                                                                                                                           |
| React            | Frontend                        | Используется для эффективного написания интерфейсов                                                                                                                                                                                              |
| Leaflet          | Frontend                        | Используется для отображения карт                                                                                                                                                                                                                                           |
| Kotlin           | Мобильное приложение            | Язык программирования, используемый для создания мобильного приложения на Android                                                                                                                                                                                            |
...
| Компонент       | Способы резервирования и отказоустойчивости |
|-----------------|---------------------------------------------|
| **NGINX**       | - **Rate limiting** для защиты от аномальных всплесков трафика.<br>- **Разные алгоритмы балансировки** в зависимости от типа трафика: `Consistent Hash` для тайлов, `Least Connections` для динамических запросов.<br>- **health-check’и** для исключения неработающих бэкендов.<br>- Размещение **в 3 AZ** на регион для устранения single point of failure. |
| **Kubernetes**  | - **Автоматический перезапуск подов** при падении контейнеров.<br>- **Horizontal Pod Autoscaler (HPA)** на основе CPU/RPS для динамического масштабирования под нагрузкой.<br>- **Pod Disruption Budgets (PDB)** для гарантии минимум работающих реплик при обновлениях. |
| **Cassandra**   | - **Multi-region replication** с `NetworkTopologyStrategy` для отказоустойчивости.<br>- **Автоматическая балансировка запросов** на уровне `gocql` драйвера (token-aware routing).<br>- **Tunable consistency**: `LOCAL_QUORUM` для чтения, `QUORUM` для записи в мастер-регионе (Нью-Йорк). |
| **PostgreSQL**  | - **Единый мастер в Нью-Йорке** — все операции записи (`review`, `place`, `map_version`) направляются только сюда.<br>- **Каскадная репликация**<br>- **Асинхронная репликация** для баланса между latency и consistency.<br>- **Резервное копирование**: ежедневный `pg_basebackup` + `WAL-G`.<br>- **pgBouncer** для мультиплексирования подключений и защиты от connection floods. |
| **Тайловый кэш**| - **Lazy rendering + MetaTile** снижает нагрузку на рендереры.<br>- **Независимые GCS-бакеты в каждом регионе** с локальным CDN. |

### Дополнительные меры

- **Graceful Shutdown**:  
  Все сервисы (Go-микросервисы, NGINX) ловят `SIGTERM`, завершают обработку активных запросов (с таймаутом) и закрывают соединения корректно.

- **Graceful Degradation**:  
  При недоступности базы или рендерера:
  - Клиент получает устаревший тайл из CDN (если есть).
  - Поиск и маршрутизация возвращают partial results (без отзывов или с упрощённой геометрией).
  - Операции записи ставятся в retry queue с экспоненциальной задержкой.

- **Мониторинг и наблюдаемость**:  
  - **Prometheus** собирает метрики по RPS, latency, error rate, CPU, памяти, размеру очередей.
  - **Grafana** отображает дашборды: глобальный трафик, статус репликации, кэш-hit rate, очередь инвалидации тайлов.
  - **Loki** для централизованного логгирования.
...
# 11. Список серверов и расчёт инфраструктуры

## 1. Конфигурации технологий (RPS/QPS **на vCPU**)

| Технология           | Характер сервиса                                   | Пропускная способность       | RAM                         |
|----------------------|----------------------------------------------------|------------------------------|-----------------------------|
| **NGINX L4**         | L4-балансер (TCP/SSL passthrough)                  | **20 000 RPS / vCPU**        | 128 МБ / vCPU               |
| **NGINX L7**         | L7-балансер (HTTP API, SSL termination)           | **5 000 RPS / vCPU**         | 256 МБ / vCPU               |
| **NGINX CDN**        | Отдача кэшированных тайлов                         | **15 000 RPS / vCPU**        | 256 МБ / vCPU               |
| **Go (gRPC, heavy)** | Маршруты, навигация, сложная бизнес-логика        | **2 000 RPS / vCPU**         | 300 МБ / vCPU               |
| **Go (CRUD, light)** | Инфо о местах, поиск, простые API                 | **8 000 RPS / vCPU**         | 150 МБ / vCPU               |
| **PostgreSQL**       | OLTP (геоданные, чтение/запись, индексы, JOIN-ы)  | **300 QPS / vCPU**           | ~64 ГБ / нода               |
| **Cassandra**        | Метаданные (гориз. масштабируемое чтение/запись)  | **4 000 QPS / vCPU**         | ~32 ГБ / нода               |

---

## 2. Пиковая нагрузка по регионам (API RPS / CDN RPS / запись)

| Регион           | Отображение карты | Информация о местах | Поиск | Маршруты | Навигация | Загрузка тайлов (CDN) | Запись (RW) |
|------------------|-------------------|---------------------|-------|----------|-----------|------------------------|------------|
| **Нью-Йорк**     | 52 100            | 8 700               | 6 200 | 3 400    | 20 800    | 41 700                 | **24**     |
| **Лос-Анджелес** | 52 100            | 8 700               | 6 200 | 3 400    | 20 800    | 41 700                 | 0          |
| **Чикаго**       | 52 100            | 8 700               | 6 200 | 3 400    | 20 800    | 41 700                 | 0          |
| **Франкфурт**    | 62 500            | 10 900              | 7 800 | 3 900    | 20 800    | 41 700                 | 0          |
| **Мумбаи**       | 62 500            | 10 900              | 7 800 | 3 900    | 27 800    | 62 500                 | 0          |
| **Пекин**        | 62 500            | 10 900              | 7 800 | 3 900    | 27 800    | 62 500                 | 0          |
| **Сан-Паулу**    | 62 500            | 10 900              | 7 800 | 3 900    | 27 800    | 21 400                 | 0          |
| **Токио**        | 62 500            | 10 900              | 7 800 | 3 900    | 27 800    | 20 100                 | 0          |

---

## 3. Расчёт по Нью-Йорку (как эталон регион)

### 3.1. Суммарная API-нагрузка (через L7)

- Отображение карты: 52 100 RPS  
- Информация о местах: 8 700 RPS  
- Поиск: 6 200 RPS  
- Маршруты: 3 400 RPS  
- Навигация: 20 800 RPS  

**Итого API:**  
`RPS_API = 52 100 + 8 700 + 6 200 + 3 400 + 20 800 = 91 200 RPS`

**CDN:**  
`RPS_CDN = 41 700 RPS`

**Общий входящий RPS через L4:**  
`RPS_L4 = RPS_API + RPS_CDN = 132 900 RPS`

Заложим:
- 15% запас на всплески (S = 1.15)
- 70% целевая утилизация CPU (U = 0.7)

### 3.2. NGINX L4 (TCP/SSL passthrough)

Производительность: **20 000 RPS / vCPU**

Формула для ядёр:
`vCPU = (RPS × S) / (RPS_на_vCPU × U)`

Подставляем:
`vCPU_L4 = (132 900 × 1.15) / (20 000 × 0.7) = 152 835 / 14 000 ≈ 10.9 ⇒ 12 vCPU`

### 3.3. NGINX L7 (HTTP API, SSL termination)

Производительность: **5 000 RPS / vCPU**

`vCPU_L7 = (91 200 × 1.15) / (5 000 × 0.7) = 104 880 / 3 500 ≈ 29.97 ⇒ 30 vCPU`

### 3.4. NGINX CDN (тайлы)

Производительность: **15 000 RPS / vCPU**

`vCPU_CDN = (41 700 × 1.15) / (15 000 × 0.7) = 47 955 / 10 500 ≈ 4.57 ⇒ 6 vCPU`

---

## 4. Go-сервисы (на vCPU)

### 4.1. Отображение карты (Go, CRUD/light)

- Нагрузка: `52 100 RPS`
- Пропускная способность: `8 000 RPS / vCPU`

`vCPU_map = (52 100 × 1.15) / (8 000 × 0.7) = 59 915 / 5 600 ≈ 10.7 ⇒ 12 vCPU`

### 4.2. Информация о местах (Go, CRUD/light)

- Нагрузка: `8 700 RPS`

`vCPU_places = (8 700 × 1.15) / (8 000 × 0.7) = 10 005 / 5 600 ≈ 1.79 ⇒ 2 vCPU`

### 4.3. Поиск (Go, CRUD/light)

- Нагрузка: `6 200 RPS`

`vCPU_search = (6 200 × 1.15) / (8 000 × 0.7) = 7 130 / 5 600 ≈ 1.27 ⇒ 2 vCPU`

### 4.4. Маршруты (Go, gRPC/heavy)

- Нагрузка: `3 400 RPS`
- Пропускная способность: `2 000 RPS / vCPU`

`vCPU_routes = (3 400 × 1.15) / (2 000 × 0.7) = 3 910 / 1 400 ≈ 2.79 ⇒ 4 vCPU`

### 4.5. Навигация (Go, gRPC/heavy)

- Нагрузка: `20 800 RPS`

`vCPU_nav = (20 800 × 1.15) / (2 000 × 0.7) = 23 920 / 1 400 ≈ 17.09 ⇒ 18 vCPU`

### 4.6. Запись (RW API)

Пусть это тяжёлый gRPC (валидации, транзакции):

- Нагрузка: `24 RPS`
- `2 000 RPS / vCPU`

`vCPU_rw = (24 × 1.15) / (2 000 × 0.7) = 27.6 / 1 400 ≈ 0.02 ⇒ 1 vCPU (минимум)`

---

## 5. Сводка CPU/памяти в Нью-Йорке (K8s-поды)

| Сервис                      | vCPU | RAM / vCPU | RAM всего | Реплик (pods) |
|-----------------------------|------|------------|-----------|---------------|
| NGINX L4                    | 12   | 0.25 ГБ    | 3 ГБ      | 12            |
| NGINX L7                    | 30   | 0.5 ГБ     | 15 ГБ     | 30            |
| NGINX CDN                   | 6    | 0.5 ГБ     | 3 ГБ      | 6             |
| Go: карта (CRUD)            | 12   | 0.15 ГБ    | 1.8 ГБ    | 12            |
| Go: места (CRUD)            | 2    | 0.15 ГБ    | 0.3 ГБ    | 2             |
| Go: поиск (CRUD)            | 2    | 0.15 ГБ    | 0.3 ГБ    | 2             |
| Go: маршруты (heavy)        | 4    | 0.3 ГБ     | 1.2 ГБ    | 4             |
| Go: навигация (heavy)       | 18   | 0.3 ГБ     | 5.4 ГБ    | 18            |
| Go: запись (RW)             | 1    | 0.3 ГБ     | 0.3 ГБ    | 1             |
| **Итого по Нью-Йорку**      | **87 vCPU** | — | **30.3 ГБ** | **87 pods**   |

---

## 6. Конфигурация K8s worker-нод и их количество (на регион)

Берём под K8s worker типичный инстанс AWS (**m6i.4xlarge** — 16 vCPU, 64 ГБ RAM).

### Нью-Йорк

- Нужно: 87 vCPU и 30.3 ГБ RAM.

`Nodes_NY = 87 / 16 = 5.44 = 6 нод (m6i.4xlarge)`

> **Распределение по AZ**: 6 нод = **2 ноды в каждой из 3 AZ**.

---

## 7. Базы данных и диск

### 7.1. PostgreSQL (геоданные, RW/RO)

Предположим:
- 1 RW-кластер в Нью-Йорке (**3 ноды**: 1 primary + 2 replica), распределены по 3 AZ.
- 1 RO-кластер на каждый крупный регион (Франкфурт, Мумбаи, Пекин, Токио) по **3 ноды**, равномерно по 3 AZ.

Тип AWS-инстанса под Postgres:
- **r6i.8xlarge**: 32 vCPU, 256 ГБ RAM.
- **r6i.4xlarge**: 16 vCPU, 128 ГБ RAM (для RO).

Диски:
- RW: **4 ТБ GP3 (3 000 IOPS, 125 МБ/с)**  
- RO: **2 ТБ GP3**

### 7.2. Cassandra (метаданные)

- RF = 3 локально, минимум **3 ноды на регион**, но для нагрузки **6 нод в тяжёлых регионах**.
- Инстанс: **i4i.2xlarge** (8 vCPU, 64 ГБ RAM, ~1.9 ТБ NVMe SSD)

---

## 8. Итоговая таблица серверов по регионам (логический уровень)

### 8.1. K8s-воркеры (m6i.4xlarge, 16 vCPU, 64 ГБ RAM)

| Регион           | vCPU (оценка по RPS) | Нод (m6i.4xlarge) | Нод на AZ |
|------------------|----------------------|-------------------|-----------|
| Нью-Йорк         | 87                   | 6                 | 2         |
| Лос-Анджелес     | 87                   | 6                 | 2         |
| Чикаго           | 87                   | 6                 | 2         |
...
| Регион       | Тип нагрузки | Нод | Тип инстанса    | RAM / нода | Диск / нода        |
|--------------|--------------|-----|-----------------|------------|--------------------|
| Нью-Йорк     | RW + RO      | 3   | r6i.8xlarge     | 256 ГБ     | 4 ТБ GP3           |
| Франкфурт    | RO           | 3   | r6i.4xlarge     | 128 ГБ     | 2 ТБ GP3           |
| Мумбаи       | RO           | 3   | r6i.4xlarge     | 128 ГБ     | 2 ТБ GP3           |
| Пекин        | RO           | 3   | r6i.4xlarge     | 128 ГБ     | 2 ТБ GP3           |
| Токио        | RO           | 3   | r6i.4xlarge     | 128 ГБ     | 2 ТБ GP3           |

> Все ноды **равномерно по 3 AZ**, по одной на зону.

---

### 8.3. Cassandra

| Регион           | Нод | Тип инстанса   | RAM / нода | Диск / нода               |
|------------------|-----|----------------|------------|---------------------------|
| Нью-Йорк         | 6   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Лос-Анджелес     | 3   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Чикаго           | 3   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Франкфурт        | 6   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Мумбаи           | 6   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Пекин            | 6   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Сан-Паулу        | 3   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |
| Токио            | 6   | i4i.2xlarge    | 64 ГБ      | ~1.9 ТБ NVMe              |

> Распределение по AZ аналогично: 3 ноды → 1 на AZ, 6 нод → 2 на AZ.

---

## 9. Итоговый расчёт стоимости на AWS

### 9.1. EC2 (месячная стоимость по on-demand)

| Инстанс        | Кол-во | Цена / час (us-east-1) | Месяц (730 ч) | Итог (USD) |
|----------------|--------|------------------------|---------------|------------|
| m6i.4xlarge    | 6×3 (NY, LA, CHI) + 6 + 8×2 + 5×2 = **48** | $1.008           | $735.84       | **$35 320** |
| r6i.8xlarge    | 3 (NY) | $2.256                 | $1 646.88     | **$4 941**  |
| r6i.4xlarge    | 3×4 = 12 | $1.128              | $823.44       | **$9 881**  |
| i4i.2xlarge    | 6×5 + 3×3 = **39** | $0.768         | $560.64       | **$21 865** |

> **Примечание**:  
> - m6i.4xlarge: $1.008/ч × 48 × 730 ≈ $35.3K  
> - r6i.8xlarge: 3 × $1.647K ≈ $4.9K  
> - r6i.4xlarge: 12 × $823 ≈ $9.9K  
> - i4i.2xlarge: 39 × $560 ≈ $21.9K  

**Итого EC2: ~$72 000 / мес**

> Для других регионов (Мумбаи, Пекин, Сан-Паулу) добавим **+20%** ⇒ **$86 400 / мес**
...
#### GP3 (PostgreSQL)
- RW (NY): 3 × 4 ТБ = **12 ТБ**
- RO (4 региона): 12 × 2 ТБ = **24 ТБ**
- Итого GP3: **36 ТБ**
- Цена: **$0.08 / ГБ / мес** → **36 000 × $0.08 = $2 880 / мес**

#### NVMe (Cassandra, i4i)
- Встроено в i4i.2xlarge — **не оплачивается отдельно**
> **Хранилище итого: ~$2 900 / мес**

---

### 9.3. Сетевой трафик (исходящий)

Пиковый исходящий трафик (см. блок «Сетевой трафик»):

- Отображение карты: **23.96 Гбит/с**
- Загрузка тайлов: **57.5 Гбит/с**
- Остальное: ~5 Гбит/с

→ **Пик глобально: ~86 Гбит/с ≈ 10.75 ГБ/с**

**средняя нагрузка = 1/3 пика ≈ 3.6 ГБ/с**

Месячный исходящий трафик:
```
3.6 ГБ/с × 3600 × 24 × 30 ≈ 9.3 PB
```

- **95% через CDN** = 8.8 PB → $0.04 → **$352 000**
- **5% через EC2** = 0.47 PB → $0.07 → **$32 900**

**Итого трафик: ~$385 000 / мес**

---

### 9.4. Итоговая месячная стоимость на AWS

| Компонент      | Стоимость, USD / мес |
|----------------|----------------------|
| EC2            | $86 400             |
| Хранилище      | $2 900              |
| Исходящий трафик (CDN + EC2) | $385 000    |
| **ИТОГО**      | **≈ $560 000 / мес** |
