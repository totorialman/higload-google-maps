...
|----------------|--------------------------------|------------------|------------------|
| **NGINX L4**   | L4-балансер (TCP/SSL passthrough) | 10 000           | 20 МБ            |
| **NGINX L7**   | L7-балансер (API, SSL termination) | 10 000           | 500 МБ           |
| **NGINX CDN**  | Отдача кэшированных тайлов     | 2 000            | 50 МБ            |
| **Go (gRPC)**  | Тяжёлые вычисления (маршруты, рендеринг, навигация) | 200 RPS/ядро | 300 МБ/ядро      |
| **Go (CRUD)**  | Метаданные (места, поиск, отзывы) | 3 500 RPS/ядро | 100 МБ/ядро      |
| **PostgreSQL** | Геоданные (RO + RW)            | 100 QPS/ядро    | 18 ГБ/реплика    |
| **Cassandra**  | Метаданные (RO)                | 1 400 QPS/ядро  | 6 ГБ/нода        |

---

## Распределение нагрузки по регионам (пиковые RPS)

| Регион         | Отображение карты | Информация о местах | Поиск | Маршруты | Навигация | Загрузка тайлов | Запись |
|----------------|-------------------|----------------------|-------|----------|-----------|------------------|--------|
...
> Запись (24 RPS глобально) обслуживается **только в Нью-Йорке**.

---

## Расчёт количества серверов (на примере Нью-Йорка)

### Шаг 1: Определение нагрузки
- **API-запросы**:  
  `52 100 + 8 700 + 6 200 + 3 400 + 20 800 = 91 200 RPS`  
- **Тайлы (CDN)**: `41 700 RPS`  
- **Запись**: `24 RPS`  

### Шаг 2: NGINX L4 (балансер балансеров)
- **Входящий трафик**: `91 200 (API) + 41 700 (CDN) = 132 900 RPS`  
- Производительность: **10 000 RPS**  
- Формула:  
  N = (RPS × S) / (RPS_инстанс × U)

  N = (132 900 × 1.15) / (10 000 × 0.7) = 152 835 / 7 000 = 21.83 = 22
- → **24 инстанса**  
- **На 3 AZ**: **8 инстансов L4 в каждой AZ**

### Шаг 3: NGINX L7 (API)
- **Нагрузка**: `91 200 RPS`  
- Производительность: **10 000 RPS**  
- Расчёт:  
  N = (91 200 × 1.15) / (10 000 × 0.7) = 104 880 / 7 000 = 14.98 = 15
- **На 3 AZ**: **5 инстансов L7 в каждой AZ**

### Шаг 4: NGINX CDN (тайлы)
- **Нагрузка**: `41 700 RPS`  
- Производительность: **2 000 RPS**  
- Расчёт:  
  N = (41 700 × 1.15) / (2 000 × 0.7) = 47 955 / 1 400 = 34.25 = 35
- → **36 инстансов**  
- **На 3 AZ**: **12 инстансов CDN в каждой AZ**

### Шаг 5: Go-сервисы (K8s Pods)

| Сервис                      | RPS   | CPU (ядер) | RAM (ГБ) | Replicas | Replicas/AZ |
|-----------------------------|-------|------------|----------|----------|-------------|
| Отображение карты (тайлы)   | 52 100| 261        | 76.8     | 12       | 4           |
| Информация о местах         | 8 700 | 3          | 0.3      | 6        | 2           |
| Поиск мест                  | 6 200 | 2          | 0.2      | 6        | 2           |
| Построение маршрутов        | 3 400 | 17         | 5.0      | 6        | 2           |
| Навигация                   | 20 800| 104        | 30.6     | 6        | 2           |
| Управление картами (RW)     | 24    | 1          | 0.1      | 3        | 1 (на AZ)   |

> RW-сервис размещён **только в Нью-Йорке**: 3 реплики (по 1 в AZ).

Аналогичные расчёты выполнены **для каждого региона** на основе его RPS.

---

## Итоговая таблица: серверы на регион

| Регион         | L4  | L7  | CDN | Отображение | Информация | Поиск | Маршруты | Навигация | Запись |
|----------------|-----|-----|-----|-------------|------------|-------|----------|-----------|--------|
| **Нью-Йорк**   | 24  | 15  | 36  | 12          | 6          | 6     | 6        | 6         | 3      |
| **Лос-Анджелес**| 24  | 15  | 36  | 12          | 6          | 6     | 6        | 6         | 0      |
| **Чикаго**     | 24  | 15  | 36  | 12          | 6          | 6     | 6        | 6         | 0      |
| **Франкфурт**  | 27  | 18  | 36  | 15          | 8          | 7     | 7        | 6         | 0      |
| **Мумбаи**     | 30  | 18  | 54  | 15          | 8          | 7     | 7        | 8         | 0      |
| **Пекин**      | 30  | 18  | 54  | 15          | 8          | 7     | 7        | 8         | 0      |
| **Сан-Паулу**  | 24  | 18  | 18  | 15          | 8          | 7     | 7        | 8         | 0      |
| **Токио**      | 24  | 18  | 18  | 15          | 8          | 7     | 7        | 8         | 0      |


---

## Итоговые затраты на аренду
