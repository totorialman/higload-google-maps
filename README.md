...
$$
N_\text{req} = \left\lceil \frac{BW_\text{пик} \times S}{BW_\text{балансировщика} \times U} \right\rceil
$$

где:  
* $BW_\text{пик}$ — пиковая пропускная способность,  
* $BW_\text{балансировщика}$ — пропускная способность одного балансировщика,  
* $U$ — целевой уровень загрузки (например, 0.7),  
* $S$ — коэффициент запаса на кратковременные всплески (например, 1.15).  

Такой подход обеспечивает высокую отказоустойчивость, выдержку пиковых нагрузок и эффективное распределение трафика.
### Итог
